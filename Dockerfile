FROM --platform=$BUILDPLATFORM node:22.23.2-alpine@sha256:c610fcdfb1d5b4740dd70c284ed3cb16bb857e0f7166196e36a5501df7a3aa32 AS build-dependencies
WORKDIR /app
RUN npm install --global npm@10.9.8
COPY package.json package-lock.json .npmrc ./
RUN npm ci

FROM --platform=$BUILDPLATFORM node:22.23.2-alpine@sha256:c610fcdfb1d5b4740dd70c284ed3cb16bb857e0f7166196e36a5501df7a3aa32 AS builder
WORKDIR /app
...
FROM node:22.23.2-alpine@sha256:c610fcdfb1d5b4740dd70c284ed3cb16bb857e0f7166196e36a5501df7a3aa32 AS runtime-dependencies
WORKDIR /app
RUN npm install --global npm@10.9.8
COPY package.json package-lock.json .npmrc ./
RUN npm ci --omit=dev
