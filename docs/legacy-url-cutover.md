...
`npm run apply:legacy-product-routes` validates that evidence, updates the URL contract, and regenerates the runtime product redirect module. The current evidence maps 101 product paths to unique `/testing-services/<id>` destinations. Four paths remain unresolved and are deliberately not routed because the evidence is absent or ambiguous:
...
Runtime redirects are resolved by `src/proxy.js` after `skipTrailingSlashRedirect` disables Next.js's automatic pre-routing slash redirect. This prevents the previous `/shop/` → `/shop` → `/testing-services` chain. Known aliases, verified product routes, transferred `/blogs/<slug>` paths, and ordinary trailing-slash canonicalization now share one resolver that preserves the query string and emits one permanent `308`. The checked-in `npm run smoke:canonical-redirects` command starts the compiled storefront, verifies representative one-hop responses with redirect following disabled, and terminates its exact child process.
